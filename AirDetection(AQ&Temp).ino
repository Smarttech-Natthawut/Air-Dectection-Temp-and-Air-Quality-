...
//1.การเรียกใช้ Library ที่เกี่ยวข้อง
  ...
  #include "DHT.h"  // เรียกใช้ Library "DHT.h" เพื่อใช้ตรวจจับอุณหภูมิ และความชื้น
  #define DHTPIN 2  // กำหนดให้ ใช้ Digital ขา 2 ของ Arduino ต่อกับ DHT
  #define DHTTYPE DHT11 // เลือก รุ่นของ Sensor DHT11 ซึ่งมีหลายรุ่น อันนี้ต้องดูครับว่าใช้รุ่นไหน


//2.กำหนดการเรียกใช้ LCD  และเรียกใช้ Sensor ตรวจวัดอุณหภูมิ (DHT) 
...
void setup() {  // กำหนดค่าการ Run Program ในครั้งแรก 
  ...
  dht.begin();                // กำหนดให้เริ่มใช้ DHT ในการตรวจจับอุณหภูมิ 
...
void loop() {                   // กำหนดให้ทำการวนซ้ำ ภายในเครื่องหมาย {}
  ...
  float t = dht.readTemperature(); // ให้ t เป้นคำทศนิยม เก็บค่าอุณหภูมิ จาก DHT Sensor
  Serial.print(t);               // แสดงค่า ที่อ่านได้จาก ตัวแปร ชือ t ออกทาง serial monitor
  Serial.println(" *C");         // แสดงค่า ออกทาง serial monitor ภายใน " " เป็น *C
...
//.......................................// ถ้า คุณภาพอากาศดี Good... // .................................................................//
  ...
  if (sensorValue < 50 || t <=30) {       // ถ้า.... ค่า sensor ที่อ่านได้ น้อยกว่า 50 หรือ อุณหภูมิ t น้อยกว่าเท่ากับ 27 ให้ทำงานในเครื่องหมาย {}
   
    lcd.setCursor(0, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 0 แถวที่ 0
    lcd.println(t); // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า Air Detection
    lcd.setCursor(4, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 0
    lcd.print(" *C (Temp)"); // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า #C Temp

    lcd.setCursor(1, 1);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 1 แถวที่ 1
    lcd.print(sensorValue);     // สั่งให้ LCD แสดง ค่า ที่ได้จาก sensor
    
    lcd.setCursor(4, 1);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 1
    lcd.print("AirQ Good...");  // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า AirQ Good...
    delay(100);                 // หน่วงเวลาไว้ 100 มิลลิเซ็ค
...
//........................................// ถ้า คุณภาพอากาศ พอใช้ได้ Fair.. //...........................................................//
  ...
  if (sensorValue >= 50 && sensorValue < 100 || t > 30 && t <= 33) {  // ถ้า.... ค่า sensor ที่อ่านได้ มากกว่าหรือเท่ากับ 35 และ น้อยกว่า 40 หรือ อุณหภูมิ t มากกว่าเท่ากับ 27 ให้ทำงานในเครื่องหมาย  {}
    lcd.setCursor(0, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 0 แถวที่ 0
    lcd.println(t);          // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า Air Detection
    lcd.setCursor(4, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 0
    lcd.print(" *C (Temp)");    // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า #C Temp
    ...
    lcd.setCursor(4, 1);                         // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 1
    lcd.print("AirQ Fair...");                   // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า AirQ Fair...
    delay(500);                                 // หน่วงเวลาไว้ 500 มิลลิเซ็ค หรือ 2 วินาที 
...
//........................................// ถ้า คุณภาพอากาศ อันตรายต่อสิ่งมีชีวิต Danger... //.................................................//
  ...
  if (sensorValue >= 100 && sensorValue < 150 || t > 33 && t <=36 ) { // ถ้า.... ค่า sensor ที่อ่านได้ มากกว่าหรือเท่ากับ 40 และ น้อยกว่า 100 หรือ อุณหภูมิ t มากกว่าเท่ากับ 35 ให้ทำงานในเครื่องหมาย {}
    lcd.setCursor(0, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 0 แถวที่ 0
    lcd.println(t);             // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า Air Detection
    lcd.setCursor(4, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 0
    lcd.print(" *C (Temp)");    // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า #C Temp
    ...
    lcd.setCursor(4, 1);                         // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 1
    lcd.print("AirQ Danger...");                 // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า AirQ Danger...
    delay(500);                                 // หน่วงเวลาไว้ 500 มิลลิเซ็ค หรือ 2 วินาที     
...
//........................................// ถ้า คุณภาพอากาศ อันตรายต่อสิ่งมีชีวิตมากๆๆ Very Danger... //.......................................//
  ...
  if (sensorValue >= 150 || t > 36 ) {                    // ถ้า.... ค่า sensor ที่อ่านได้ มากกว่าหรือเท่ากับ 100 หรือ อุณหภูมิ t มากกว่าเท่ากับ 35 ให้ทำงานในเครื่องหมาย {}
    lcd.setCursor(0, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 0 แถวที่ 0
    lcd.println(t);          // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า Air Detection
    lcd.setCursor(4, 0);        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 4 แถวที่ 0
    lcd.print(" *C (Temp)");    // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า #C Temp
    ...
    lcd.setCursor(3, 1);                        // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 3 แถวที่ 1
...
//.......................................// ถ้าไม่ตรงตาม if ข้างบน ให้ทำตาม else {} //........................................................//
    ...
    digitalWrite(pin13, LOW);         //  สั่งให้ pin13 มีค่าเป็น LOW หรือเรียกว่า OFF ให้ตัดไฟ
    lcd.setCursor(0, 0);              // กำหนด ให้ LCD แสดงผล  คอลัมน์ที่ 0 แถวที่ 0
    lcd.print("Temp & AirQ NotM");    // สั่งให้ LCD แสดงข้อความภายในเครื่องหมาย (" ") ว่า Temp & AirQ NotM    
    lcd.clear();    
  }
